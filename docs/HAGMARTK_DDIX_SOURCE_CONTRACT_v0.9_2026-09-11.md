...
## 16. v0.9 — Santa Confluence Candidate

### 16.1 Provenance boundary
The primary Didi/CST material supports a high-quality context formed by a classic Didi needle, DMI/ADX trend context and Bollinger opening. The explicit label "Operação Santa" is better supported by secondary educational material that adds TRIX and Stochastic agreement.

Therefore this checkpoint freezes only a versioned **research confluence candidate**. It must not be represented as an official mathematical formula authored directly by Didi Aguiar.

Provenance label:
`PRIMARY_DIDI_CONTEXT_PLUS_SECONDARY_SANTA_LABEL_CANDIDATE`.

### 16.2 Deterministic research confluence
Implemented in:
`backend/strategies/ddix/santa_confluence.py`

The candidate starts from a valid classic 3/8/20 needle and evaluates five aligned evidence channels:
- classic needle direction;
- DMI direction and trend presence;
- Bollinger objective opening candidate;
- ADX rising at the current observation;
- TRIX and Stochastic line bias aligned to the needle direction.

The confluence levels are intentionally separated:
- `NOT_QUALIFIED`: no classic needle;
- `NEEDLE_ONLY`: classic needle without the primary contextual alignment;
- `PRIMARY_CONTEXT_ALIGNED`: needle + DMI direction + trend present + Bollinger opening;
- `SANTA_CONFLUENCE_CANDIDATE`: primary context + ADX rising + TRIX aligned + Stochastic aligned.

The classifier is symmetric for bullish and bearish cases. It uses current line bias for TRIX/Stochastic alignment and does not infer profitability from an extreme reading.

### 16.3 Safety boundary
Even when all semantic conditions are present:
`semantic_santa_candidate = True`

Operational promotion remains hard blocked:
`operationally_approved = False`

The unresolved dependencies are persisted in every result:
...
### 16.4 Validation status
Directed Santa tests cover:
- bullish full alignment;
- bearish full alignment;
- primary-context alignment without secondary directors;
- rejection when DMI direction conflicts;
- rejection when Bollinger is not opening;
- hard operational block even for semantic Santa.

Current research status:
- `SANTA_SEMANTIC_CONFLUENCE = RESEARCH_CANDIDATE`;
- `SANTA_OPERATIONAL_PROMOTION = BLOCKED`;
- Telegram DDIX = OFF;
- PAPER DDIX = OFF;
- real-order execution = NOT AUTHORIZED.

No DVP, ORB or TDC runtime rule is modified by this checkpoint.

### 16.5 Next gate
Do not add another indicator merely to increase confluence count. The next DDIX work must reduce the open parity debt: obtain reproducible platform outputs for the Didi Index and auxiliary buffers, then compare the research calculations against those outputs. Only after the relevant parity gates pass may a later contract decide whether the Santa candidate can qualify prospective DDIX events.
