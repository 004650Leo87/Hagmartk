...
def test_refresh_persists_provider_support_and_marks_states(tmp_path):
    ...
    assert unsupported == ["BTCUSD", "ETHUSD"]
    assert len(scanner.provider_supported_assets) * len(SHADOW_TIMEFRAMES) == 77
...
def test_unsupported_provider_failures_do_not_pollute_active_coverage(tmp_path):
    ...
    assert global_row["configured_combinations"] == 91
    assert global_row["provider_supported_combinations"] == 77
    assert global_row["provider_unsupported_combinations"] == 14
