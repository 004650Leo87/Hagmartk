...
$backendCommand = "Set-Location '$root'; `$env:HAGMARTK_AUTOSTART='1'; `$env:HAGMARTK_MARKET_ADAPTER='unified'; `$env:HAGMARTK_CYCLE_THEORY_SHADOW='1'; `$env:HAGMARTK_CORS_ORIGINS='http://127.0.0.1:$frontendPort,http://localhost:$frontendPort'; python -m uvicorn backend.api.app:app --host 127.0.0.1 --port $backendPort"
