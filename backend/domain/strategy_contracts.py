...
def build_product_strategy_registry() -> StrategyContractRegistry:
    ...
    cycle_v111 = StrategyContract(
        ...
        publication_capability=PublicationCapability.GATED,
        publication_gate_id="CYCLE_THEORY_PRIVATE_SHADOW_ALERT_GATE_V1",
        allowed_event_classes=(
            MarketEventClass.RADAR,
            MarketEventClass.RESEARCH_UPDATE,
            MarketEventClass.QUANT_EVENT,
        ...
        real_order_execution_allowed=False,
        limitations=(
            "Prospective Shadow observation is enabled; private alerts are evidence notifications, not trade recommendations.",
            "Sunday weekly-open policy uses M15 until the first paper order; subsequent observation uses M5. 24/7 instruments remain M5.",
            "Observed Bid/Ask lifecycle is PAPER only; broker acceptance, slippage, commission and swap remain partial/modelled.",
    ...
    orb_v1 = StrategyContract(
        ...
        publication_capability=PublicationCapability.GATED,
        publication_gate_id="ORB_PRIVATE_SHADOW_ALERT_GATE_V1",
        allowed_event_classes=(MarketEventClass.RADAR, MarketEventClass.RESEARCH_UPDATE, MarketEventClass.QUANT_EVENT),
        evidence_keys=("ORB_V1_CONFORMANCE_EVIDENCE", "ORB_V1_SHADOW_EVIDENCE"),
        real_order_execution_allowed=False,
        limitations=(
            "Frozen v1 signal/risk rules; no parameter optimization is authorized.",
            "Prospective Shadow is enabled for explicit Binance USD-M perpetual session profiles only.",
